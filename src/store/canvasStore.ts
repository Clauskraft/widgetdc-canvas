...
import type { CanvasNodeData, CanvasNodeType, ProvenanceData } from '../components/nodes';
...
// --- G1: Auto-Population Pipeline — map Neo4j properties → Canvas visual properties ---
...
export const useCanvasStore = create<CanvasState>((set, get) => ({
  ...
  loadFromGraph: async (id) => {
    ...
    try {
      ...
      const nodes: Node[] = (nodeRecords ?? []).map((rec) => {
        ...
        // G2: Reconstruct provenance from flat properties
        ...
        // G2: Reconstruct thinkingSteps from JSON string
        ...
        // G1: Enrich visual properties from Neo4j labels + props
        ...
        return {
          ...
          data: {
            ...
            // Visual properties: saved values take precedence, then G1 enrichment
  ...
  // --- Canvas 5X: Graph Exploder (progressive, 20-node limit) ---
  expandNode: async (nodeId, relTypes, depth = 1) => {
    ...
    try {
      ...
      // Fetch with higher limit to know total, but only show PROGRESSIVE_EXPAND_LIMIT
      ...
      // Track expand state for "Load More"
      ...
      // Position new nodes radially around parent
  ...
  groupSelected: () => {
    // Group all selected nodes (multiselect) into a combo node
    // For now, use all nodes matching a name pattern or a simple selection
    const { nodes, edges, selectedNodeId } = get();
    if (!selectedNodeId) return;
    ...
    // Hide grouped nodes, remove internal edges, keep external edges pointing to combo
  ...
  ungroupCombo: (comboId) => {
    // Placeholder — would restore hidden nodes from metadata
    const { nodes } = get();
    const combo = nodes.find(n => n.id === comboId);
    if (!combo || combo.type !== 'combo') return;
    // In a full implementation, we'd restore from metadata.childIds
    // For now, just remove the combo node
    get().pushSnapshot();
    set({
  ...
  // --- Strategic 10: Toast bridge ---
  _toast: undefined,
  setToast: (fn) => set({ _toast: fn } as Partial<CanvasState>),

  // --- Strategic 1: Auto-Analysis on Entity ---
  autoAnalyze: async (nodeId) => {
    ...
    try {
      ...
      // Process reasoning result → thought node
      ...
      t?.('success', `Auto-analysis complete: ${nodeCount} nodes generated for "${label}"`);
    } catch (err) {
  ...
  // --- Strategic 3: Narrative Generation ---
  generateNarrative: async () => {
    ...
    try {
      // Build canvas summary for /reason
      ...
      const edgeLabels = edges.map(e => (e as Edge & { label?: string }).label).filter(Boolean);

      const canvasSummary = [
        `Canvas contains: ${nodes.length} nodes, ${edges.length} edges.`,
      ...
      // Create artifact node with the narrative
      const artId = get().addNodeWithData('artifact', {
  ...
  // --- Strategic 5: Hypothesis-Evidence Scoring ---
  evaluateHypothesis: async (thoughtNodeId) => {
    ...
    try {
      // Find all connected evidence nodes
      ...
      // Update thought node with score
      set({
        nodes: get().nodes.map(n => n.id === thoughtNodeId ? {
          ...n, data: {
            ...n.data,
        ...
        // Color edges: green (supports), red (refutes), gray (neutral)
  ...
  // --- Strategic 7: Cross-Engagement Pattern Discovery ---
  discoverPatterns: async () => {
    ...
    try {
      ...
      // Fetch strategic insights grouped by domain
      ...
      // Send to /reason for pattern synthesis
  ...
  loadTemplate: async (templateId) => {
    ...
    try {
      ...
      // Apply engagement canvas columns if defined
      if (template.columns && template.columns.length > 0) {
        const colNodes = get().nodes;
        const positioned = colNodes.map(n => {
          const nType = n.type ?? 'entity';
          // Find which column this node type belongs to
          for (const [colName, colDef] of Object.entries(ENGAGEMENT_COLUMNS)) {
            if (colDef.nodeTypes.includes(nType as CanvasNodeType)) {
              const colIdx = colDef.nodeTypes.indexOf(nType as CanvasNodeType);
              const existingInCol = colNodes.filter(cn =>
                ENGAGEMENT_COLUMNS[colName]?.nodeTypes.includes(cn.type as CanvasNodeType)
              ).indexOf(n);
              return {
                ...n,
                position: { x: colDef.x + 20, y: 80 + existingInCol * 100 },
              };
            }
          }
          return n;
        ...
        const { nodes, edges } = get();
        set({ nodes: applyDagreLayout(nodes, edges), layoutMode: 'mindmap' });
      }

      // Auto-analyze first entity if enabled
  ...
  recordStep: (actionType, nodeContext, outcome) => {
    ...
    ).catch(() => { /* silent — engagement tracking is best-effort */ });
  },
  ...
  recommendNextActions: async (nodeId) => {
    ...
    const nType = String(node.type ?? 'entity');
    const connectedEdges = get().edges.filter(e => e.source === nodeId || e.target === nodeId);
    const recommendations: ActionRecommendation[] = [];

    // Rule-based recommendations based on node type and state
    if (connectedEdges.length === 0) {
      recommendations.push({
        action: 'expand', label: 'Expand neighbors', confidence: 0.95,
        reasoning: 'Node has no connections — expanding will reveal graph context',
        targetNodeId: nodeId,
      });
    }
    if (nType === 'entity' || nType === 'agent') {
      recommendations.push({
        action: 'matchTenders', label: 'Match tenders', confidence: 0.8,
        reasoning: `${nType} nodes often have relevant tender opportunities`,
        targetNodeId: nodeId,
      });
    }
    if (nType === 'thought' || nType === 'insight') {
      recommendations.push({
        action: 'evaluateHypothesis', label: 'Evaluate hypothesis', confidence: 0.85,
        reasoning: 'Score this insight against available evidence',
        targetNodeId: nodeId,
      });
    }
    if (connectedEdges.length > 3) {
      recommendations.push({
        action: 'crossReference', label: 'Cross-reference', confidence: 0.7,
        reasoning: 'Well-connected node may have cross-domain patterns',
        targetNodeId: nodeId,
      });
    }
    if (get().nodes.length >= 5) {
      recommendations.push({
        action: 'narrative', label: 'Generate narrative', confidence: 0.6,
        reasoning: 'Enough nodes to produce a meaningful analysis report',
      });
    }
    recommendations.push({
      action: 'autoAnalyze', label: 'Auto-analyze', confidence: 0.75,
      reasoning: `Run full analysis pipeline on "${label}"`,
      targetNodeId: nodeId,
    });
  ...
  exportAuditTrail: async (format) => {
    ...
    const trail = nodes.map(n => {
      ...
      return {
        ...
        provenance: prov ?? { createdBy: 'manual', createdAt: 'unknown', source: 'unknown' },
    ...
    const provenanceSources = [...new Set(trail.map(t => t.provenance.createdBy))];
    lines.push('', `## Integrity Summary`, '');
    lines.push(`- **Sources**: ${provenanceSources.join(', ')}`);
    lines.push(`- **Manual nodes**: ${trail.filter(t => t.provenance.createdBy === 'manual').length}`);
    lines.push(`- **Auto-generated nodes**: ${trail.filter(t => t.provenance.createdBy !== 'manual').length}`);
    return lines.join('\n');
  },
  ...
  runAnalysisPipeline: async (entityName, templateId) => {
    ...
    try {
      ...
      t?.('success', `Pipeline complete: ${result.nodeCount} nodes, narrative generated`);
      return result;
    } catch (err) {
